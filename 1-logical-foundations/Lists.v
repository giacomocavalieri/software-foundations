...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | n :: t => if even n then oddmembers t else n :: oddmembers t 
  end.
...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with
  | nil => 0
  | n :: ns => if n =? v then 1 + count v ns else count v ns
  end.
...
Fixpoint member (v : nat) (s : bag) : bool :=
  match s with
  | nil => false
  | n :: ns => if n =? v then true else member v ns
  end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  match s with
  | nil => nil
  | n :: ns => if n =? v then ns else n :: remove_one v ns
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | n :: ns => if n =? v then remove_all v ns else n :: remove_all v ns
  end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | n :: ns => member n s2 && included ns (remove_one n s2)
  end.
...
Theorem add_inc_count : forall b : bag, forall n : nat,
  count n (add n b) = 1 + count n b.
Proof.
  intros b n. simpl. rewrite eqb_refl. reflexivity.
Qed.
...
Proof.
  intros l1 l2. induction l1 as [ | n ns IH ].
...
Theorem rev_involutive : forall l : natlist,
  rev (rev l) = l.
Proof.
  intros l. induction l as [ | n ns IH ].
  - reflexivity.
...
Proof.
  intros l. induction l as [ | n ns IH ].
  - reflexivity.
  - simpl. rewrite eqb_refl. simpl. rewrite IH. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros s. induction s as [ | n ns IH ].
  - reflexivity.
  - simpl. destruct n as [ | m ].
...
Proof.
  intros b1 b2 n. induction b1 as [ | x xs IH ].
  - reflexivity.
  - simpl. destruct (x =? n).
    + simpl. rewrite IH. reflexivity.
    + simpl. rewrite IH. reflexivity.
...
Proof.
  intros l default. destruct l as [ | n ns ].
  - reflexivity.
  - reflexivity.
Qed.
(** [] *)
